...
const NFTItem = () => {
  ...
  const { getERC20Contract, getERC721Contract, getERC1155Contract } =
    useNFTContract()
  const {
    getSalesContract,
    buyItemETH,
  ...
  const [bundleSalesContractApproved, setBundleSalesContractApproved] =
    useState({})
  const [auctionContractApproved, setAuctionContractApproved] = useState(false)
  const [auctionContractApproving, setAuctionContractApproving] =
    useState(false)
  ...
  const getBundleInfo = async () => {
    ...
    try {
      ...
      const _bundleListing = await getBundleListing(data.bundle.owner, bundleID)
      if (_bundleListing) {
  ...
  const itemSoldHandler = async (
    ...
    if (eventMatches(nft, id)) {
      ...
      const token = getTokenByAddress(paymentToken)
      const _price = parseFloat(ethers.utils.formatUnits(price, token.decimals))
  ...
  const bundleListedHandler = (
    ...
    if (bundleID.toLowerCase() === _bundleID.toLowerCase()) {
      ...
      const price = parseFloat(ethers.utils.formatUnits(_price, token.decimals))
  ...
  const addEventListeners = async () => {
    ...
    auctionContract.on('UpdateAuctionStartTime', auctionStartTimeUpdatedHandler)
  ...
  const handleRevealContent = async () => {
    ...
    try {
      ...
      try {
        ...
        const msg = `Approve Signature on Rena.io with nonce ${nonce}`
    ...
    try {
      ...
      if (bundleID) {
        ...
        if (allowance.lt(amount)) {
          const tx = await erc20.approve(Contracts[chainId].bundleSales, amount)
          await tx.wait()
        }
  ...
  return (
    <div className={styles.container}>
              ...
              <div className={styles.panelWrapper}>
                <Panel title="Items" icon={ViewModuleIcon} expanded>
                  <div className={styles.items}>
                    {(loading ? [null, null, null] : bundleItems.current).map(
                      (item, idx) => renderBundleItem(item, idx)
                    )}
                  </div>
                </Panel>
              </div>
      ...
      <Menu
        ...
        onClose={handleClose}
        classes={{
          paper: styles.shareMenu,
          list: styles.shareMenuList,
        }}
        anchorOrigin={{
          vertical: 'bottom',
          horizontal: 'right',
